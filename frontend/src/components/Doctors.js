...
export default function DoctorDashboard() {
  ...
  const fetchExistingPrescriptions = async (patientId) => {
    ...
    try {
      ...
      const response = await fetch(`https://hospital-management-seven-lake.vercel.app/api/doctor/prescriptions/${patientId}`, {
        headers: {
  ...
  const fetchDoctorProfile = async () => {
    try {
      ...
      const response = await fetch('https://hospital-management-seven-lake.vercel.app/api/doctor/profile', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchPatientsWithAppointments = async () => {
    try {
      ...
      const response = await fetch('https://hospital-management-seven-lake.vercel.app/api/doctor/patients-with-appointments', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await fetch('https://hospital-management-seven-lake.vercel.app/api/doctor/appointments', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const renderPatientManagement = () => {
    ...
    const fetchAvailableSlots = async (patientId, date) => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://hospital-management-seven-lake.vercel.app/api/doctor/available-slots?patientId=${patientId}&date=${date}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
    ...
    const handleDeletePrescription = async (prescriptionId) => {
      if (window.confirm('Are you sure you want to delete this prescription?')) {
        try {
          const token = localStorage.getItem('token');
          const response = await fetch(`https://hospital-management-seven-lake.vercel.app/api/doctor/prescriptions/${prescriptionId}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`
            }
      ...
      if (selectedAction === 'prescribe-medication') {
        try {
          ...
          const url = appointmentData.prescriptionId
            ? `https://hospital-management-seven-lake.vercel.app/api/doctor/prescriptions/${appointmentData.prescriptionId}`
            : 'https://hospital-management-seven-lake.vercel.app/api/doctor/prescribe-medication';
        ...
        try {
          const token = localStorage.getItem('token');
          const response = await fetch('https://hospital-management-seven-lake.vercel.app/api/doctor/schedule-appointment', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
